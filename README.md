...
## Inference status & known issues

### CPT model — recommended for current use

The CPT-merged model (`output/cpt/merged/merged`) works well for domain Q&A.
Use the `Q1:/A1:` few-shot completion format to prime the base model:

```bash
source .venv-vllm/bin/activate
python -m vllm.entrypoints.openai.api_server \
    --model output/cpt/merged/merged \
    --dtype bfloat16 \
    --max-model-len 8192 \
    --port 8000
```

Query via the **completions** endpoint (not chat):
```bash
curl http://localhost:8000/v1/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "output/cpt/merged/merged",
    "prompt": "Q1: Deine Frage?\nA1:",
    "max_tokens": 200,
    "temperature": 0.1,
...
### SFT model — known quality issues (TODO)

The SFT-merged model (`output/sft/merged`) suffers from hallucination and repetition
loops. **Root cause:** the QA pairs in step 08 were generated by the CPT base model,
which itself hallucinated. SFT learned those patterns.

**Planned fix:** Regenerate `data/processed/sft_qa_llm.jsonl` using a high-quality
instruct model (e.g. via Mistral or OpenAI API) and retrain SFT from `output/cpt/checkpoint-2400`.
The CPT domain knowledge is solid — only the SFT data quality needs fixing.
